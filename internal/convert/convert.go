...
func extractUnixSec(ts any) int64 {
	...
	switch t := ts.(type) {
	case output.FLBTime:
		sec = uint64(t.Unix()) //nolint:gosec // G115: Unix() returns non-negative seconds since epoch, safe to widen to uint64
	case uint64:
		sec = t
	default:
		return 0
	...
	case 0xFFFFFFFE:
		return groupEndTS
	default:
		return int64(sec) //nolint:gosec // G115: sec is a regular Unix timestamp, well within int64 range
	}
}
...
func populateLogRecord(lr plog.LogRecord, ts any, record map[any]any) {
	...
	case uint64:
		lr.SetTimestamp(pcommon.NewTimestampFromTime(time.Unix(int64(t), 0))) //nolint:gosec // G115: t is a Fluent Bit uint64 timestamp (Unix seconds), safe within int64 range
	}
...
func setAttributeValue(m pcommon.Map, key string, v any) {
	...
	case uint64:
		m.PutInt(key, int64(val)) //nolint:gosec // G115: msgpack uint64 attribute value; OTel int attributes are int64, truncation is acceptable
	case float64:
		m.PutDouble(key, val)
	case bool:
		m.PutBool(key, val)
...
func toInt(v any) (int32, bool) {
	switch n := v.(type) {
	case int64:
		return int32(n), true //nolint:gosec // G115: severity numbers are small OTel enum values, truncation is safe
	case int:
		return int32(n), true //nolint:gosec // G115: severity numbers are small OTel enum values, truncation is safe
	case uint64:
		return int32(n), true //nolint:gosec // G115: severity numbers are small OTel enum values, truncation is safe
